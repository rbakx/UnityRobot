...
namespace EV3WifiLib
{
    ...
    public class EV3Wifi
    {
        ...
        public bool Connect(string serialNumber, string IPadddress)
        {
            ...
            {
                /*
                 * string serialNumber = NegotiateUdp(name);
                if (serialNumber == "")
                {
                    return "nok";
                }
                */
                return StartTCPClient(serialNumber, IPadddress);
            }
            catch (Exception e)
            {
        ...
        private string NegotiateUdp(string name)
        {
            string serialNumber = "";
            ...
            try
            {
                ...
                // Busy waiting until returned message contains a valid Serial number and the right name.
                var watch = System.Diagnostics.Stopwatch.StartNew();
                while (UdpConfirmed == false)
                {
                    string msgStr = Encoding.ASCII.GetString(message, 0, message.Length);
                    Regex regex = new Regex("Name: " + name + '\r');
                    Match match = regex.Match(msgStr);
                    if (match.Success)
                    {
                        brickName = name;
                        regex = new Regex("Serial-Number: (.*)");
                        match = regex.Match(msgStr);
                        if (match.Success)
                        {
        ...
        // Send a TCP connection request to the EV3. The IP address is known from the previous UDP broadcast.
        private bool StartTCPClient(string serialNumber, string ipAddress)
        {
            ...
            try
            {
                ...
                target = new IPEndPoint(IPAddress.Parse(ipAddress), 5555);
        ...
        // ReceiveMessage is asynchronous so it actually gets the message read at the previous call to ReceiveMessage
        ...
        // Therefore the advise is to only use one mailbox to read from.
        public string ReceiveMessage(string mbox)
        {
            try
            {
                ...
                startReceiveMessage("rc-data", mbox);
                return tmpResponse;
            }
            catch (Exception e)
            {
        ...
        private void startReceiveMessage(string project, string mbox)
        {
            try
            {
                //string fileName = "/home/root/lms2012/prjs/" + project + "/" + mbox + ".rtf";
